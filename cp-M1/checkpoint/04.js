...
function contarPacientes(pacientes, count = 0) {
  if(!pacientes.length) return count === 0 ? 'No hay pacientes en la planta' : `La cantidad de pacientes en la planta son: ${count}`;
  pacientes.pop()
  return contarPacientes(pacientes, ++count)
}

console.log(contarPacientes(["aldo", "camilo"]))
